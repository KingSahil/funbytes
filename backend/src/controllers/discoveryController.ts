...
export class DiscoveryController {
  ...
  getTrending = async (req: Request, res: Response): Promise<void> => {
    ...
    const trendingItems = [...mockFeedData]
      .sort((a, b) => {
  ...
  search = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      let results = mockFeedData.filter((item) => {
  ...
  getVisualSection = async (req: Request, res: Response): Promise<void> => {
    try {
      const { type } = req.params; // 'cartoons' or 'memes'
      const items = mockFeedData.filter(
        (i) => i.contentType === (type === 'cartoons' ? 'cartoon' : 'meme')
      );
