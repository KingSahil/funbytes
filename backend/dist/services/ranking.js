    ...
    rankFeed(items, options = {}) {
        ...
        const scoredItems = items.map((item) => {
            ...
            if (userInterests.length > 0) {
                ...
                const matches = userInterests.filter((interest) => itemTagsLower.includes(interest) || item.category.toLowerCase() === interest).length;
            ...
            const rawEngagement = (item.engagement?.likes || 0) +
