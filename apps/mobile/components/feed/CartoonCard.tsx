...
export const CartoonCard: React.FC<CartoonCardProps> = ({ item }) => {
  ...
  const authorName = item.meta?.cartoonist || item.author || 'Editorial Cartoonist';
  const authorHandle = `@${(item.meta?.publication || item.sourceName).toLowerCase().replace(/[^a-z0-9]/g, '')}`;
        ...
        <View style={styles.contentColumn}>
          ...
          {item.imageUrl ? (
            <View style={styles.mediaContainer}>
              <Image source={{ uri: item.imageUrl }} style={styles.artwork} resizeMode="contain" />
            </View>
          ) : null}
